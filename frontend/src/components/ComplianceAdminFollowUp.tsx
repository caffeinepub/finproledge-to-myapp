...
export default function ComplianceAdminFollowUp() {
  ...
  const clearFilters = () => {
    setFilters({ title: '', client: '', status: '', dueDate: '' });
  ...
  const flatRows = useMemo(() => {
    return (followUps ?? []).map((f) => ({
      ...f,
      _clientStr: f.clientPrincipal?.toString() ?? '',
      _statusStr: String(f.status),
      _dueDateStr: formatDate(f.dueDate),
    }));
  }, [followUps]);
  ...
  const filteredRows = useMemo(() => {
    ...
    if (filters.dueDate) filterMap['_dueDateStr'] = filters.dueDate;
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between gap-2 flex-wrap">
        <h3 className="text-lg font-semibold">
          All Follow-Ups ({sortedRows.length}
          {sortedRows.length !== (followUps?.length ?? 0) && ` of ${followUps?.length ?? 0}`})
        </h3>
        {hasActiveFilters && (
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
        ...
        <div>
          <Label className="text-xs text-muted-foreground mb-1 block">Filter by Due Date</Label>
          <Input
      ...
      {sortedRows.length === 0 ? (
        <p className="text-muted-foreground text-sm py-4 text-center">
          {hasActiveFilters ? 'No follow-ups match the current filters.' : 'No follow-up items found.'}
        </p>
      ) : (
            ...
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="min-w-[120px]">
                  <SortableHeader label="Due Date" columnKey="dueDate" sortState={sortState} onSort={handleSort} />
                </TableHead>
            ...
            <TableBody>
              {sortedRows.map((item) => (
                <TableRow key={String(item.id)}>
                  ...
                  <TableCell className="text-xs">{item._dueDateStr}</TableCell>
                  <TableCell>
