...
actor {
  ...
  // Approved users and admins can view analytics.
  ...
  // Any authenticated caller may read their own profile (needed before approval).
  ...
  // Admin-only: look up any profile by principal.
  ...
  // Any authenticated caller may save their own profile (needed during registration
  // before the approval workflow completes).
  ...
  // Public: no authentication required (backwards-compatible visitor request).
  ...
  // Public: no authentication required. Mandatory lead capture fields must be
  // non-empty; the lead record is stored alongside the service request.
  public shared ({ caller }) func createVisitorRequestWithLead(input : LeadRequestInput) : async Nat {
    if (input.fullName == "" or input.businessName == "" or input.phoneNumber == "") {
      Runtime.trap("All lead capture fields are required (fullName, businessName, phoneNumber)");
    };
  ...
  // Admin-only: view all leads captured via the lead-gate form.
  ...
  // Approved users or admins; returns only the caller's documents.
  ...
  // A user may view their own documents; admins may view any client's documents.
  ...
  // ── Compliance deliverables (admin-managed) ───────────────────────────────

  // Admin-only: create a compliance deliverable for a client.
  ...
  // A user may view their own deliverables; admins may view any client's.
  ...
  // Approved users or admins; returns only the caller's deliverables.
  ...
  // A user may view their own pending deliverables; admins may view any client's.
  ...
  // Approved users or admins; returns only the caller's pending deliverables.
  ...
  // Approved users may update their own requests; admins may update any.
  ...
  // A user may view their own requests; admins may view any client's.
  ...
  // Approved users or admins; returns only the caller's requests.
  ...
  // ── Client deliverables (submitted by clients / admins on behalf of clients) ──
  ...
  // Approved users or admins may submit deliverables.
  ...
  // Admin-only: submit a deliverable on behalf of a specific client.
  ...
  // Admin-only: accept or reject a client-submitted deliverable.
  // (Status transitions such as #accepted / #rejected are admin decisions.)
  ...
  // Approved users or admins; returns only the caller's submitted deliverables.
  ...
  // Admin-only: view submissions for a specific client.
  ...
  // Approved users or admins; returns only the caller's payments.
  ...
  // Any caller (including unauthenticated / anonymous) may check their own
  // approval status.
  ...
  // Any caller may request approval (this is the registration entry point).
  ...
  // Admin-only: list all pending / processed approval requests.
  ...
  // Admin-only: create a to-do (optionally assigned to a client).
  ...
  // Approved users or admins; returns only the caller's assigned to-dos.
  ...
  // Approved users may update their own to-dos; admins may update any.
  ...
  // Approved users only: create their own to-do.
  ...
  // Approved users or admins; returns only the caller's timeline entries.
  ...
  // Approved users may update their own timelines; admins may update any.
  ...
  // Approved users only: create their own timeline entry.
  ...
  // Approved users or admins; returns only the caller's follow-ups.
  ...
  // Approved users may update their own follow-ups; admins may update any.
  ...
  // Approved users only: create their own follow-up.
