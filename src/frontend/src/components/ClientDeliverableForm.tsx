...
export default function ClientDeliverableForm() {
  ...
  const handleFileChange = (file: File | null) => {
    ...
    // Auto-fill title with the original filename (including extension) if the user
    // hasn't typed a title yet. This preserves the extension so MIME detection works
    // correctly when the admin downloads the file.
    if (file && title.trim() === "") {
      setTitle(file.name);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Use the original filename as the title if none provided, preserving the
    // file extension so the admin can download in the correct format.
    const effectiveTitle = title.trim() || selectedFile.name;
    ...
    try {
      ...
      toast.success("Deliverable submitted successfully!", {
        description: `"${effectiveTitle}" has been submitted and is awaiting review.`,
      });
  ...
  const canSubmit = !!selectedFile && !submitDeliverable.isPending;
  ...
  return (
    <div className="bg-card border border-border rounded-lg p-6 mb-6 shadow-sm">
      ...
      <p className="text-xs text-muted-foreground mb-5">
        Upload your completed work for review. Accepted formats: PDF, XLSX,
        DOCX, CSV, ZIP, and images.
      </p>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="space-y-1.5">
          ...
          <Input
            ...
            placeholder="e.g. Q1 2026 Financial Report (auto-filled from filename)"
