...
export default function PaymentForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(
        err?.message ??
          "An error occurred while submitting your payment request.",
      );
    }
  };
      ...
      <CardContent className="p-6">
        {success && (
          <Alert className="mb-4 border-emerald-200 bg-emerald-50">
            ...
            <AlertDescription className="text-emerald-800 text-sm">
              Payment request submitted successfully. Our team will process it
              shortly.
            </AlertDescription>
          </Alert>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <p className="text-xs text-muted-foreground bg-muted/40 rounded p-3 border border-border">
            <strong className="text-foreground">Note:</strong> Submitting this
            form records your payment request with our team. We will contact you
            to complete the transaction securely. Payments are received via
            PayPal.
          </p>
