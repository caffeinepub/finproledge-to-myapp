...
import { AlertCircle, CreditCard, Mail, Receipt } from "lucide-react";
...
function PaymentStatusBadge({ status }: { status: PaymentStatus }) {
  ...
  return (
    <span
      className={`inline-flex items-center px-2.5 py-0.5 rounded text-xs font-semibold border ${styles[status] ?? "bg-gray-100 text-gray-600 border-gray-200"}`}
    >
      ...
      {paymentSettings?.paypalEmail ? (
        ...
        <Alert className="border-amber-200 bg-amber-50">
          ...
          <AlertDescription className="text-amber-800 text-sm">
            No PayPal account configured. Go to <strong>Settings</strong> tab to
