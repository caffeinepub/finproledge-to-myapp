...
export default function TrafficAcquisitionSection({
  sources,
}: TrafficAcquisitionSectionProps) {
  const chartData =
    sources.length > 0
      ? sources.map((s) => ({
          name: s.source,
            ...
            <ResponsiveContainer width="100%" height={180}>
              <PieChart>
                <Pie
                  ...
                  labelLine={false}
                >
                  {NEW_VS_RETURNING.map((entry, index) => (
                    <Cell
                      key={entry.name}
                      fill={NR_COLORS[index % NR_COLORS.length]}
                    />
                  ))}
                </Pie>
                <Tooltip
              ...
              {NEW_VS_RETURNING.map((item, i) => (
                <div key={item.name} className="flex items-center gap-1.5">
                  <span
                    ...
                    style={{ backgroundColor: NR_COLORS[i] }}
                  />
              ...
              {LANDING_PAGES.map((lp, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell className="py-2.5 px-4 text-xs font-mono text-muted-foreground">
                      {lp.page}
                    </TableCell>
