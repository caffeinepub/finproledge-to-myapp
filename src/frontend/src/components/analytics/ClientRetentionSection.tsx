...
import type { ClientRetentionMetrics } from "../../backend";
...
const FUNNEL_STEPS = [
  ...
  {
    step: "Login Success",
    label: "User successfully authenticates",
    users: 680,
  },
  {
    step: "Document Upload",
    label: "User navigates to upload section",
    users: 420,
  },
  {
    step: "Upload Complete",
    label: "User successfully uploads a document",
    users: 310,
  },
  {
    step: "Form Submission",
    label: "User submits a service request",
    users: 210,
  },
];
...
export default function ClientRetentionSection({
  data,
  ...
  const scaledFunnel = FUNNEL_STEPS.map((step, idx) => {
    const baseDropRate =
      idx === 0
        ? 0
        : (FUNNEL_STEPS[idx - 1].users - step.users) /
          FUNNEL_STEPS[idx - 1].users;
    return {
      ...step,
    ...
    <section className="space-y-4">
      <div className="flex items-center gap-2 mb-2">
        ...
        <h2 className="text-lg font-serif font-semibold text-foreground">
          Client Retention &amp; Portal Usage
        </h2>
      </div>
      <p className="text-sm text-muted-foreground mb-4">
        Track how often existing clients return and identify friction points in
        the client portal.
      </p>
        ...
        <Card className="border border-border shadow-sm">
          ...
          <CardContent className="p-5">
            ...
            <div className="space-y-2">
              {[
                {
                  label: "Login Drop-off",
                  pct: 17,
                  desc: "Users who fail to complete login",
                },
                {
                  label: "Upload Abandonment",
                  pct: 26,
                  desc: "Users who start but don't finish uploading",
                },
                {
                  label: "Form Incompletion",
                  pct: 33,
                  desc: "Users who leave mid-form submission",
                },
              ].map((item) => (
                <div key={item.label} className="space-y-1">
                  <div className="flex items-center justify-between">
                    ...
                    <span
                      className={`text-xs font-bold ${item.pct >= 30 ? "text-red-600" : item.pct >= 20 ? "text-amber-600" : "text-emerald-600"}`}
                    >
                      {item.pct}% drop-off
                    </span>
                  </div>
          ...
          <Table>
            <TableHeader>
              <TableRow className="border-b border-border bg-muted/20">
                ...
                <TableHead className="text-xs font-semibold uppercase tracking-wider text-muted-foreground py-2 px-4 text-right">
                  Users
                </TableHead>
                <TableHead className="text-xs font-semibold uppercase tracking-wider text-muted-foreground py-2 px-4 text-right">
                  Drop-off
            ...
            <TableBody>
              {scaledFunnel.map((step, idx) => (
                <TableRow
                  key={step.step}
                  className={`border-b border-border hover:bg-muted/20 ${idx % 2 !== 0 ? "bg-muted/10" : ""}`}
                >
                  ...
                  <TableCell className="py-2.5 px-4 text-sm text-right font-semibold text-foreground">
                    {step.users.toLocaleString()}
                  </TableCell>
                  <TableCell className="py-2.5 px-4 text-right">
                    {step.dropRate !== null ? (
                      <span
                        className={`text-xs font-bold ${Number.parseFloat(step.dropRate) >= 30 ? "text-red-600" : Number.parseFloat(step.dropRate) >= 15 ? "text-amber-600" : "text-emerald-600"}`}
                      >
                        -{step.dropRate}%
                      </span>
                    ) : (
