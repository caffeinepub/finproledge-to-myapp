...
export default function TechnicalPerformanceSection({
  webVitals,
}: TechnicalPerformanceSectionProps) {
  const perf = webVitals.performance > 0 ? webVitals.performance : 0.82;
  const accessibility =
    webVitals.accessibility > 0 ? webVitals.accessibility : 0.94;
  const bestPractices =
    webVitals.bestPractices > 0 ? webVitals.bestPractices : 0.88;
  ...
  const vitals = [
    ...
    },
    {
      ...
      score: accessibility,
      value:
        accessibility >= 0.9 ? "0.04" : accessibility >= 0.5 ? "0.18" : "0.42",
    ...
    <section className="space-y-4">
      <div className="flex items-center gap-2 mb-2">
        ...
        <h2 className="text-lg font-serif font-semibold text-foreground">
          Technical Performance (Web Vitals)
        </h2>
      </div>
      <p className="text-sm text-muted-foreground mb-4">
        Core Web Vitals, mobile-friendliness, and error monitoring for a healthy
        site.
      </p>
        ...
        {vitals.map((v) => (
          <Card key={v.key} className="border border-border shadow-sm">
            <CardContent className="pt-5 pb-4 px-5">
              ...
              <p className={`text-3xl font-bold mt-2 ${scoreColor(v.rating)}`}>
                {v.value}
              </p>
              ...
              <TableBody>
                {ERROR_MONITORING.map((err, idx) => (
                  ...
                  >
                    ...
                    <TableCell className="py-2 px-4 text-xs font-mono text-muted-foreground max-w-[160px] truncate">
                      {err.url}
                    </TableCell>
                    <TableCell className="py-2 px-4 text-sm text-right font-semibold text-foreground">
                      {err.count}
                    </TableCell>
                  </TableRow>
                ))}
