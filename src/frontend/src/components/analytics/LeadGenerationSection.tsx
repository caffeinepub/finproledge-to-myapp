...
import { FileText, PhoneCall, Star } from "lucide-react";
import React from "react";
import type { LeadGenerationMetrics } from "../../backend";
...
export default function LeadGenerationSection({
  ...
  const submissionRate =
    total > 0
      ? ((formSubmissions / (formSubmissions + 420)) * 100).toFixed(1)
      : "19.2";
  ...
  return (
    <section className="space-y-4">
      ...
      <p className="text-sm text-muted-foreground mb-4">
        Track how visitors convert into leads — from form submissions to direct
        contact actions.
      </p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <Card className="border border-border shadow-sm">
          <CardContent className="pt-5 pb-4 px-5">
            ...
            <p className="text-xs text-muted-foreground mt-1">
              of visitors complete a quote or consultation form
            </p>
          </CardContent>
        </Card>
        ...
        <Card className="border border-border shadow-sm">
          <CardContent className="pt-5 pb-4 px-5">
            ...
            <div className="flex items-end gap-3 mt-2">
              <p className="text-4xl font-bold text-foreground">
                {clickToCallCount.toLocaleString()}
              </p>
              <span className="text-sm text-muted-foreground mb-1">
                mobile interactions
              </span>
            ...
            <p className="text-xs text-muted-foreground mt-3">
              Mobile users who tapped your phone number or email link directly
              this period.
            </p>
              ...
              {leadCategories.map((cat, idx) => (
                ...
                >
                  ...
                  <TableCell className="py-3 px-4">
                    <Badge
                      variant="outline"
                      className={`text-xs border ${cat.color}`}
                    >
                      {cat.type}
                    </Badge>
                  </TableCell>
