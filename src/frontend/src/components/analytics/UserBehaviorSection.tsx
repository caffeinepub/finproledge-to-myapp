...
export default function UserBehaviorSection({
  ...
  const avgSeconds =
    sessionMetrics.averageTimeOnPage > 0
  ...
  const pageViews =
    Number(sessionMetrics.pageViews) > 0
      ? Number(sessionMetrics.pageViews).toLocaleString()
      : "10,920";

  const sessions =
    Number(sessionMetrics.sessions) > 0
  ...
  return (
    <section className="space-y-4">
      ...
      <p className="text-sm text-muted-foreground mb-4">
        Move beyond clicks to see how people actually interact with your
        content.
      </p>
        ...
        <Card className="border border-border shadow-sm">
          ...
          <CardContent className="p-5 space-y-4">
            <p className="text-xs text-muted-foreground">
              Percentage of users who scroll to each depth zone.
            </p>
