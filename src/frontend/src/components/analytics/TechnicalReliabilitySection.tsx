...
import type { TechnicalReliabilityMetrics } from "../../backend";
...
function getSpeedBadgeClass(ms: number): string {
  if (ms < 2000)
    return "bg-emerald-100 text-emerald-800 border border-emerald-200 hover:bg-emerald-100";
  if (ms <= 4000)
    return "bg-amber-100 text-amber-800 border border-amber-200 hover:bg-amber-100";
...
export default function TechnicalReliabilitySection({
  data,
}: TechnicalReliabilitySectionProps) {
  ...
  const securityScore = sslStatus
    ? Math.min(95, 70 + Math.round(mobileScore * 0.25))
    : 30;
  ...
  return (
    <section className="space-y-4">
      ...
      <p className="text-sm text-muted-foreground mb-4">
        Financial clients expect a secure, professional experience — monitor the
        ...
        <Card className="border border-border shadow-sm">
          <CardContent className="pt-5 pb-4 px-5">
            ...
            <div className="flex items-end gap-2 mt-2">
              <p
                className={`text-4xl font-bold ${getMobileScoreColor(mobileScore)}`}
              >
                {mobileScore}
                <span className="text-base font-normal text-muted-foreground">
                  /100
        ...
        <CardContent className="p-5">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {[
              ...
              <div
                key={item.label}
                className="flex items-center gap-2.5 p-2.5 rounded-lg bg-muted/30 border border-border"
              >
                <span
                  className={`text-base ${item.pass ? "text-emerald-600" : "text-red-500"}`}
                >
                  {item.pass ? "✓" : "✗"}
                </span>
