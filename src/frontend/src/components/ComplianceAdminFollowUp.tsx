...
function ClientCell({ principalStr }: { principalStr: string }) {
  const { data: profile, isLoading } =
    useGetUserProfileByPrincipal(principalStr);
...
export default function ComplianceAdminFollowUp() {
  ...
  const handleSort = (key: string) => {
    setSortState((prev) =>
      toggleSortDirection(prev.column, prev.direction, key),
    );
  };
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between gap-2 flex-wrap">
        <h3 className="text-lg font-semibold">
          ...
          {sortedRows.length !== (followUps?.length ?? 0) &&
            ` of ${followUps?.length ?? 0}`}
          )
        </h3>
        {hasActiveFilters && (
                ...
                <TableRow key={String(item.id)}>
                  ...
                  <TableCell>
                    {item.clientPrincipal ? (
                      <ClientCell
                        principalStr={item.clientPrincipal.toString()}
                      />
                    ) : (
