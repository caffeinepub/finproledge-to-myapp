...
function getStatusLabel(status: DeliverableStatus): string {
  switch (status) {
    case DeliverableStatus.drafting:
      return "Drafting";
...
function getTypeLabel(type: DeliverableType): string {
  switch (type) {
    case DeliverableType.consulting:
      return "Consulting";
    case DeliverableType.monthly:
      return "Monthly";
    case DeliverableType.annual:
      return "Annual";
    case DeliverableType.quarterly:
      return "Quarterly";
    default:
      return "General";
  }
...
export default function DeliverableCard({ deliverable }: DeliverableCardProps) {
  ...
  const rejectMutation = useMutation({
    ...
    onError: () => {
      toast.error(
        "Rejection feature coming soon. Please contact your advisor.",
      );
    },
  });
  ...
  return (
    <Card
      className={`border transition-all ${isUrgent && !isOverdue ? "border-amber-400 bg-amber-50/30 dark:bg-amber-950/10" : isOverdue ? "border-red-400 bg-red-50/30 dark:bg-red-950/10" : "border-border"}`}
    >
        ...
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex flex-wrap items-center gap-2 mb-2">
              ...
              <span
                className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium border ${getStatusColor(deliverable.status)}`}
              >
