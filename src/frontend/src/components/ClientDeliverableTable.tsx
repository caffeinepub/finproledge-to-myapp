...
export default function ClientDeliverableTable() {
  ...
  const handleStatusChange = async (
    ...
    try {
      ...
      toast.success(
        `Deliverable ${newStatus === ClientDeliverableStatus.accepted ? "approved" : "rejected"}`,
      );
    } catch {
        ...
        <TableBody>
          {deliverables.map((d) => (
            <TableRow key={String(d.id)}>
              ...
              <TableCell>
                <StatusBadge
                  status={d.status as unknown as ClientDeliverableStatus}
                />
              </TableCell>
              <TableCell>
                <div className="flex gap-2">
                  <Button
                    ...
                    disabled={
                      ...
                      (d.status as unknown as string) ===
                        ClientDeliverableStatus.accepted
                    }
                    onClick={() =>
                      handleStatusChange(d.id, ClientDeliverableStatus.accepted)
                    }
                  >
                    {updateStatus.isPending ? (
                  ...
                  <Button
                    ...
                    onClick={() =>
                      handleStatusChange(d.id, ClientDeliverableStatus.rejected)
                    }
                  >
                    {updateStatus.isPending ? (
                      <Loader2 className="w-3 h-3 animate-spin" />
